...
<script language="Jscript" runat="server">
	...
	function main(conn){
		...
		if(!row.Eof){
			...
			all.play = row("Play").value.replace(/_/g,"x").replace(/,/,"&nbsp;&nbsp;");
			...
			for(var p in froms["code"]){
				...
				if(plans!="NO PLAN" && plans!=null){
					...
					for(var i=0;i<m.length;i++){
						...
						//Response.write(ttp+"|"+kv[0]+"|"+typeof _vf);
						if(_vf.length == 1){
					...
					if(that!="" && that!="undefined"){
						for(var p in that){
							...
							if(that[p] in plan[c[p_c]]){
								planStr += "<i class='x_plan x_win'>"+_vf[1]+"["+plan[c[p_c]][that[p]]+"]</i>";
							}else{
								planStr += "<i class='x_plan'>"+_vf[1]+"</i>";
							}
						}
					}
				}
			...
			var bankMod = function(obj){
				return '<tr class="my">'
					+'<td>'+obj.name+'***</td>'
					+'<td><span class="eng">￥'+obj.allmoney+' <%=setting(56)%></span></td>'
					+'<td><span class="eng">￥'+obj.buymoney+' <%=setting(56)%></span></td>'
					+'<td><span class="eng">￥'+obj.sca+'%</span></td>'
					+'<td><span class="eng">'+obj.addtime+'</span></td>'
					+'<td><span class="eng">￥'+obj.winmoney+' <%=setting(56)%></span></td>'
				+'</tr>';
			}
