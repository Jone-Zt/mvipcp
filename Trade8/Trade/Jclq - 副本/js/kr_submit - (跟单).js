var football_submit = function(map,game){
	...
	if ( ISHM ){
	    postdata['tc']     = document.querySelector("#royalty").value;
		 postdata['ishm']  = document.querySelector("input[name='gd']:checked").value;
	 }
	 ...
	 console.log(dd);
	 window.$.post("/Trade/360/data/buy.asp?"+new Date().getTime(),{ pt:dd,order:orderid.join("|").toString()},function(data){
	    alert(data.msg);
		if(data.code == 1 ){
