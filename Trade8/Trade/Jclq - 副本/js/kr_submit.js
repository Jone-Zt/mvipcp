var football_submit = function(map,game){
	...
	if ( ISHM ){
		...
		postdata['ishm']   = ISHM;
		postdata["rg"]     = document.querySelector("#meRG").value;
		postdata["bd"]     = document.querySelector("#meBD").value;
	 }
...
var getplayway = function(){
   ...
   href       =  href.split("/");
   var index =   href[href.length-1].replace('.html','');
...
window.$(function(){
   window.$("#meRG").on("input",function(){
        var total = Number(window.$("#ownMoney").text());
		var indata = Number(window.$(this).val());
		window.$("#meBD").val((total - indata));
   })
})