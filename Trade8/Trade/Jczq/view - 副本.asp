...
<body>
	...
	<div class="main">
		...
		<div class="lot_nav" style="background:url(../../Images/faxq/faxqul.png) no-repeat left 10px center; height:100px;">
            ...
            <br>
            <div class="lot_user mleft"><span class="fontcss1">自投金额</span><br><span class="fontcss2"><%response.Write(all.buymoney&setting(56))%></span></div><div class="lot_user"><span class="fontcss1">起跟金额</span><br><span class="fontcss2"><%response.Write(all.onemoney&setting(56))%></span></div>
        </div>
		<div class="lot_context">
			...
			<div class="lot_view">
				...
				<div class="lot_all_user">
					<span class="fontcss1">跟单信息：</span>共<span class="fontcss2"><%response.Write(all.gdzj&setting(56))%></span><div id="gdlbs" style="float:right; background:#E81929; color:#FFFFFF; width:100px; height:40px; text-align:center; line-height:40px;">跟单列表 ></div>
				</div>
				<div class="lot_all_user">
					<span class="fontcss1">提　　成：</span><span class="fontcss2"><%response.Write(all.tc&"%")%></span>
				</div>
				<div class="lot_all_user">
					<span class="fontcss1">状　　态：</span><span class="fontcss2"><%If all.overtime<now() Then response.Write("方案已截止") else response.Write("进行中")%></span>
				</div>
			</div>
		</div>
	...
	<div class="head">
        ...
        <%else%>
            <div class="lot_open lot_gd_input" style=" font-size:14px; font-weight:bold;color:#E81929;">购买　<input type="text" placeholder="1" value="1"> 倍　<font id="am"><%=all.onemoney%></font><%=setting(56)%>　<button data-id="<%=all.id%>" class="btn_hm_buy">　跟　　单　</button></div>
        <%end If%>
	</div>
	<script>
		...
		$(".lot_gd_input input").keyup(function(){
			var am=<%=all.onemoney%>*$(".lot_gd_input input").val();
			$("#am").html(am);
		});
