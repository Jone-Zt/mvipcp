...
var now  = Db.FormatDate("Y/m/d H:i:s");
 if( act  == "getdata" ){
	 var pp = Request("p");
	 pp  =  pp == "" ? 1 : pp  ;
     var data = Db.PageData(pp,15,"select Id,Num,Mul,gdbs,UserName,OverTime,BuyMemo,play,'¾º²Ê×ãÇò' as jctype,DateTime from kr_football_buy where hm=2 and OverTime>'"+now+"' union all select Id,Num,Mul,gdbs,UserName,dealtime,betcontent,play,'¾º²ÊÀºÇò' as jctype,DateTime from kr_basketball_buy where copysheet=1 and deadtime>'"+now+"' order by overtime desc");
	 for( var d = 0 ; d < data["data"].length ; d++ ){
		...
		//data["data"][d]["DateTime"]=ttt.replace(/\-|\s|\:/g,'');
	 }
