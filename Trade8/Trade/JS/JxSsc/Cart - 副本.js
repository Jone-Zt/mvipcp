		...
		if(localStorage["storage258001"]&&localStorage["storage258001mul"])
		{
			...
			for(var j=0;j<k1.length;j++)
			{
				...
				for(var i=0;i<k2.length;i++)
				{
					...
					if(i==2){if(codes=="") codes=codes+wanfa+k2[i]; else codes=codes+"$"+wanfa+k2[i];}
				}
    ...
    n.mobile.codeViewInit = function() {
        ...
        if (e) {
            for (var l = 0; l < s; l++) {
                ...
                tmp = r[l].split("|");
                o = tmp[2];
                tmp[0] == "DD" ? (u = tmp[2].replace(/,/g,"").split(""), o = n.mobile.bszs["b" + u[0]] + n.mobile.bszs["b" + u[1]]) : tmp[0].indexOf("F") > -1 && tmp[0] != "F2" && (c = tmp[1] > 1 ? "\u5305\u53f7": "\u5355\u5f0f");
                f = a.replace("{pt}", tmp[0]).replace("{code}", tmp[2]).replace("{count}", tmp[1]).replace("{name}", n.mobile.gname[tmp[0]] + c + "\uff1a").replace("{codeC}", o);
                i.push(f);
            }
        ...
        t("#pay_buy").on({
            click: function() {
                ...
                if (!r) {
                    ...
                    !1;
                    if (!t("input[name='pettreaty']").prop("checked")) return n.lottery.alert("\u8bf7\u5148\u9605\u8bfb\u5e76\u540c\u610f\u300a\u8d2d\u5f69\u534f\u8bae\u300b\uff01"),
                    !1;
                    if (!t("input[name='petrisk']").prop("checked")) return n.lottery.alert("\u8bf7\u5148\u9605\u8bfb\u5e76\u540c\u610f\u300a\u98ce\u9669\u987b\u77e5\u300b\uff01"),
                    !1;
				...
				function ttpanduan()
				{
					...
					{	
						if(zuihaoNum){
								...
								if((Number(nowExpect)+Number(zuihaoNum)) > 120){
									alert("\u4eca\u5929\u8ffd\u53f7\u8fd8\u5269 "+(120-nowExpect)+" \u671f");
									return;
								}
						}
