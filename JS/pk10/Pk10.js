...
//console.log(type,numtype);
		 ...
		 for( ; s <= num ; s++ ){
		    var index = han[s];
//console.log(this.code[index].sort().join(","));
	...
	_pro_.lookSelectCode= function(){
		...
		ds("input[name='total']").val(numText*2);
		ds("input[name='totalh']").val(numText*2);
		ds("input[name='BuyCount']").val( numText*2/2 );
		ds("input[name='BdCount']").val( numText*2/2 );
		var mf=ds("input[name='totalh']").val()*$("input[ name='beishu']").val()/$("input[ name='total']").val();
		mf=mf.toFixed(2);
		ds("#one-m").html("份，每份"+mf+danwei);
		ds(".Add_div").show();
	}
	...
	_pro_.SuccessCleaer = function(){
		...
		for( var index in this.code ){
			this.code[index].splice(0,this.code[index].length);
		}
	...
	_pro_.toSubmit = function(){
	   ...
	   if( this.IsHm == 0 ){
		   ...
		   tosend['Total']    = ds('input[name="total"]').val();
		   tosend['ticheng']  = ds('input[name="ticheng"]').val();
		   tosend['show']     = ds('input[name="isshow"]:checked').val();
	   }
