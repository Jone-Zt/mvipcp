...
$(function(){
   ...
   $("input[name='total']").on("change",function(){
       var mf=$("input[name='totalh']").val()*$("input[ name='beishu']").val()/$("input[ name='total']").val();
	   mf=mf.toFixed(2);
	   $("#one-m").html("份，每份"+mf+danwei);
	   $('input[name="BdCount"]').val(Number($('input[name="total"]').val())-Number($('input[name="BuyCount"]').val()));
   })
   ...
   $("input[name='beishu']").change(function(){
	   ...
	   var zfs=$("input[ name='totalh']").val()*$("input[ name='beishu']").val();
	   $("input[ name='total']").val(zfs);
       var mf=zfs/$("input[ name='total']").val();
	   mf=mf.toFixed(2);
	   $("#one-m").html("份，每份"+mf+danwei);
	   $('input[name="BdCount"]').val(Number($('input[name="total"]').val())-Number($('input[name="BuyCount"]').val()));
   })
   ...
   $('button[class="btn-red"]').on('click',function(){
	...
	cc['code'] = cc['code'].replace(/大/g,"A").replace(/小/g,"B").replace(/单/g,"C").replace(/双/g,"D");
	  ...
	  tosend['show']    = $('input[name="isshow"]:checked').val();
...
function echo(obj){
 ...
 $('input[name="BuyCount"]').on('change',function(){
	var total = $('input[name="total"]').val();
	if ( Number(this.value) > total ){
		this.value = total ;
	}
    $('input[name="BdCount"]').val(Number(total)-Number(this.value));	
 })
})