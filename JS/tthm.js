...
cxnumber = 1;
       ...
       function cx1(caizhong)
	   {
		   ...
		   if ( typeof(caizhong) == 'number'){
		    caizhong    = escape(document.getElementById("tstyle"+caizhong).innerHTML);
		   }
		   caizhong = caizhong == -1 ? '' : caizhong ;
			$.ajax({
				type: "POST",
...
val += "</div></h4></div><ul class='hm_list_box hm_list_tp'><li class='color_red'>&nbsp;&nbsp;"+_this.money+" / "+(_this.money/_this.all).toFixed(2).replace('.00','')+" "+coinType+"</li><li>";
...
val += "<li>"+_this.ssy+"</li></ul><a href='./Include/Project_Info.asp?id="+_this.id+"'><ul class='hm_list_box color_999 font_10'><li>&nbsp;&nbsp;总"+coinType+" / 每份</li><li>保底</li><li>剩余份数</li></ul></a></dd></dl>";
            ...
            success: function(data){
                ...
                if(d.err == 1){
                    ...
                    if(d.err == 2){
                        ...
                        alert("投注错误(您的【"+coinType+"】不足或【投注份额】超出最大投注数)！");
                    }else if(d.err == 6){
                        alert("投注失败，投注已截止！");
                    }else if(d.err == 0){
