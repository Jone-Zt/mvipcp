...
<%
  ...
  BdChuli(data[0]['orderid']); //合买保底处理
  var planlist = Db.getData(sql);
  if ( planlist.length < 1 ) Db.Echo("竞猜足球第"+data[0]['orderid']+"期已派奖");
  for(var i = 0 ; i < planlist.length  ; i++ ){
    if(!IsAllout(planlist[i]['BuyId'],planlist[i]['id'])) continue;
	 ...
	 if( planlist[i]['last'] == 1 ){
		...
		backsql  = "update KR_Bank_Back set issuccess='"+issuccess+"',win_cost="+winmoney+",win_cost_get="+prizemoney+",tc_money="+tc+",back_state='已处理' where LotteryID='"+planlist[i]['BuyId']+"'";
  ...
  */
  ...
  function getNewReg(regarr,RegList){
     var newReg  = [] ;
     for( var tt=0 ; tt < regarr.length ; tt++ ){
	   for(var ttt = 0 ; ttt < RegList.length ; ttt++ ){
	      if(regarr[tt] == RegList[ttt]['username']){
		    newReg.push(RegList[ttt]);
		  }
	   }
	 } 
	 return newReg;
