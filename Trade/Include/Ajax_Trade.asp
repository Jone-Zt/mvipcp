...
Function hmlist()
	...
	AddTime = array("and DATEDIFF(D,a.addtime,GETDATE())<10","and DATEDIFF(D,datetime,GETDATE())<3")
	If IsNum(AddT) Then
	...
	If LottName<>"" Then
		LottName = "and a.lotterytype='"&KR(unescape(LottName))&"'"
	Else
		LottName = ""
	End If
	...
	conn.Execute("insert into #temp_hemai(id,lotteryName,UserName,allMoney,zg,sy,OverTime,winMoney,bd,state,lotteryid,onemoney,addtime,jd) select top(200)a.id,lotterytype,UserName,allMoney,anumber,anumber-hnumber,CASE WHEN LEN(Lottery_Num)=5 THEN b.endtime ELSE convert(varchar,a.addtime,23)+' '+convert(varchar,b.endtime,8) END AS overtime,wincost,baodinum,isOver,lotteryid,onemoney,a.addtime,CASE WHEN a.schedule<100 THEN 1 ELSE 0 END from KR_Buy a,KR_Lottery_Issue b WHERE ishm='1' and isreturn='0' "&IsUserName&" "&LottName&" "&AddTime(0)&" "&ztSelect(0)&" "&jeSelect(0)&" "&jdSelect(0)&" "&bdSelect(0)&" AND right(a.expect,len(b.Lottery_Num))=b.Lottery_Num AND b.Lottery_Name=a.lotterytype order by addtime desc")
	...
	if LotterySetting(74)="1" then
	  If LottName = "and a.lotterytype='北京赛车'" Or LottName="" Then 
	    conn.Execute("INSERT INTO #temp_hemai(id,lotteryName,UserName,allMoney,zg,sy,OverTime,winMoney,bd,state,lotteryid,onemoney,addtime,jd) select top(200)id,lotterytype as lotteryname,UserName,allMoney,anumber as zg ,(anumber-hnumber) as sy,isjz as overtime,wincost as winmoney,baodinum as bd,isOver,lotteryid,onemoney,addtime,CASE WHEN schedule<100 THEN 1 ELSE 0 END as jd from KR_Buy WHERE ishm='1' and isreturn='0' and lotterytype='北京赛车' order by id desc")
	  end if
	end if
