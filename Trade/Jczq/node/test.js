var dbc = require('./configTest.js').Dbconfig;
        ...
        cl.each(function(k){
            if(k>1){
				...
				*/
                ...
                te = te.replace(/\s+/g,"");
                var te2 = te.match(/^(.+?)\((.+?)\)VS(.+)$/);
                json.orderid = e.attr('gamedate')+e.attr('gamenum');
                ...
                if(!err){
                    if(rec[0]["count"]==0){
                        M.insertInto(sqlStr+" SELECT @@IDENTITY as newid FROM KR_Football_Code",function(rec){
                            /*NetWork.get("http://localhost:85/Trade/jczqPrize/p1.asp?id="+rec[0]["newid"],function(){//路径----派奖
...
M.connMssql();
var N = new NetWork({url:'http://www.vipcp.cn/trade/jczq/kj/kj.asp',timec:1});
N.start();