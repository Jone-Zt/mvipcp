...
<body>
<ul id="header">
     <li><a href="/"><i class="iconfont" style="font-size:.8rem;"></i></a></li>
     <li style="font-size:.8rem;">跟单</li>
     <li></li>
</ul>
<ul class="data-list" >
  <li>
 ...
 <script type="text/html" id="DataList">
    ...
    <p class="faqiren">
	   ...
	   <%
			...
			%>
   <span>&nbsp;{{if obj.pic!="" && obj.pic!=null}}
	
	<img src="/{{obj.pic}}" height="40" width="40"  style="border-radius:50%"> 
	{{else}}
	<img src="http://m.588micai.com/profilepicture/12345.png" height="40" width="40"   style="border-radius:50%"> 
	{{/if}}<%=address%>{{obj.address}}</span>
    ...
    <table class="matchbox" cellpadding="0" cellspacing="0">
      <tr>
        <td>类型</td>
        <td>自购金额(元)</td>
        <td style="border-right:2px solid #d6d6d6">单倍金额(元)</td>
        <td rowspan="2"><a href="/{{if obj.jctype=="竞彩足球"}}Trade/Jczq/View.asp{{else}}trade/jclq/view.htm{{/if}}?id={{obj.Id}}" class="gdbtn">跟单</a></td>
      </tr>
      <tr>
        <td>{{obj.jctype}}</td>
  ...
  <br>
  <hr style="height:1px;border:none;border-top:7px solid #E0E0E0;" />
  {{/each}}
 </script>
