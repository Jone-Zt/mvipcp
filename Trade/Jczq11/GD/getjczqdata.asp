...
var now  = Db.FormatDate("Y/m/d H:i:s");
 if( act  == "getdata" ){
	 var pp = Request("p");
	 pp  =  pp == "" ? 1 : pp  ;
     var data = Db.PageData(pp,15,"select a.Id,a.Num,a.Mul,a.gdbs,a.UserName,a.OverTime,a.BuyMemo,a.play,'¾º²Ê×ãÇò' as jctype,a.DateTime,b.pic,b.address from kr_football_buy a left join kr_user b on a.UserName=b.UserName where hm=2 and OverTime>'"+now+"' union all select a.Id,a.Num,a.Mul,a.gdbs,a.UserName,a.dealtime,a.betcontent,a.play,'¾º²ÊÀºÇò' as jctype,a.DateTime,b.pic,b.address from kr_basketball_buy a left join kr_user b on a.UserName=b.UserName where copysheet=1 and deadtime>'"+now+"' order by overtime desc");
