	...
	t.extend(e, {
		...
		quickInit: function() {
			var e = this.config,
				...
				n = /(\S+)Selector$/;
			for (var o in e)
				if ("string" == a.getType(o) && n.test(o)) i[RegExp.$1] = t(e[o]);
			this.betAreaInit().betMethodInit().loadData();
			this.common.init();
			return this
		},
		myInit: function() {
			t(i).on("ortchange", function(t) {
		...
		betAreaInit: function() {
			var e = this,
				...
				i, n = this.cache.boxCenter;
			i = Class.Game.COMS.JC.BetArea.create({
				gameWrapSelector: "#wrapper",
				gameTypes: this.config.gameTypes,
				teamData: this.config.teamData
			});
			i.initMoreOption = function() {
				var e = i,
					a;
				this.cache.gameWrap.delegate(".more-option[data-matchcode]", "click", function(e) {
					var a = t(this),
						n = a.attr("data-matchcode"),
						c, s, r = i.cache.gameInfo,
						h = i.cache.selectGameInfo;
					if (n && r[n] && r[n].spData) {
						s = h[n];
						t.dialog({
							title: null,
							css: "select-dialog bqc-dialog",
							width: "93.125%",
							height: "17.5rem",
							content: t.template(t("#bqcDialogTpl").html(), {
								teamData: i.config.teamData,
								gameInfo: r[n],
								selectInfo: s || {}
							}),
							button: [],
							init: function() {
								var e = t(this),
									a = this.id;
								e.delegate(".cancel", "click", function(e) {
									t.dialog(a)
								}).delegate(".confirm", "click", function(o) {
									t.dialog(a);
									i.del(n);
									e.find(".betbtn.active[data-gametype]").each(function() {
										var e = t(this),
											...
											o = +e.attr("data-index");
										if (o >= 0 && a && n) i.selectOption(n, a, o)
									})
								}).delegate(".betbtn[data-gameType]", o, function(e) {
									var a = t(this);
				...
				i.onChange(function(t) {
					...
					var e = this.get().selectGameInfo[t],
						a, i = this.get().gameInfo[t],
						n, o, c = this.config.teamData.bqc,
						s = i.ele.find(".more-option"),
						r = [],
						h = [],
						o = "点击选择上半场和全场结果的组合";
					if (s.hasClass("no-play")) return;
					if (e && e.bqc) {
						o = "";
				...
				i.onClear(function() {
					if (!a || !a.length) a = i.cache.gameWrap.find(".more-option");
					a.not(".no-play").removeClass("active").html("点击选择上半场和全场结果的组合")
				})
			};
			i.initMoreOption();
			this.betArea = i;
		...
		renderHTML: function() {
			...
			var s = function() {
				var s = {},
					r = [],
					h, l, d, m, d = document.createDocumentFragment();
				l = e.matchList;
				...
				r.forEach(function(e, o) {
					...
					m = t.format(n, {
						...
						dateNum: s[e].length,
						dayMatchList: t.template(i, s[e])
					});
					m = t(m);
					d.appendChild(m[0])
				});
				o.append(d);
				c.betArea.addMatch(l)
			};
			s();
		...
		loadData: function() {
			...
			this.get(c, {}, function(n, c) {
				...
				try {
					...
					a.gameEn = c.gameEn;
					o.matchList = s.explainData(c, i.config.gameTypes);
					if (!s.checkList4Bet(o.matchList)) {
						o.noData.removeClass("hide");
						o.loadDataStatus = 1;
						t.sendMsg("loadData", 1);
						return
