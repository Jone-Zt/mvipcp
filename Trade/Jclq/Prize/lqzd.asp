...
<%
  id = JczqPrize.GET("id");
  var sql  = "select orderid,[plan] from KR_BasketBall_Code where orderId ="+id ;
  ...
  if ( planlist.length < 1 ) Db.Echo("竞猜篮球第"+data[0]['orderid']+"期已派奖 success");
  ...
  */
  ...
  function OneListRebate(obj,orderid,playname){
	 ...
	 for( var j=0 ; j < needdoList.length ; j++ ){
      ...
      Db.doExecute(sql);
      var tcmx = {'LotteryID':orderid,'UserName':needdoList[j]["U"],'LotteryName':'竞猜篮球','LotteryType':playname,'Issuccess':'已提成','back_money2':fdq,'addtime':Db.FormatDate('Y-m-d H:i:s'),'back_state':'已处理','follows':'系统返点','b_befor':needdoList[j]["Uer"],'b_after':needdoList[j]["Uer"]+fdq,'Anumber':0,Hnumber:0};
