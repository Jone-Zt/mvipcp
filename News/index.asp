...
function cx()
{
	$.ajax({
		...
		success: function(data){
			...
			{
				j=j+"<div class='article main-index' articleid='257' id='articleId257'><a href='/News/newsxq.asp?xq="+$(err[i]).attr("id")+"'><h3>"+unescape($(err[i]).attr("title")).substring(0,7)+"<em class='times'>"+$(err[i]).attr("adddate").substring(5,10)+"</em></h3></a></div>";		
			}
