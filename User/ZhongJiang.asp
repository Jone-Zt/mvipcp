...
<body>
<%checksssion()%>
<div class="head"><h2 id="navtit">中奖记录</h2><a href="javascript:history.go(-1);" class="btn-qgkj" id="sourceUrl"><span><em></em></span>返回</a></div>
<div class="wap_h" id="list_data">
	<div class="ex_record_box">
        <div id="div_tz" class="brts_box">
            <div class="filter_s" id="showpro">
            <div align="center"><p class="time_txt"><%lottidSelect%>&nbsp;&nbsp;<input type="text" value="<%=Format_Time(now(),2)%>" class="basic_txt gray" id="beginday" name="beginday" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd',onpicked:function(){do_search();}})"></p></div>
					...
					else if($(err).attr("type")=="1")
					{
						...
						if($(data).find("row").length>0)
						{
							...
							else
							{
							    ...
							    var url  = "/Trade/Include/Project_Info.asp?id=";
								if ($(this).attr("lotname") == "竞彩足球" ){
								    url = "/Trade/Jczq/view.asp?id=";
								}else if ($(this).attr("lotname")=="竞彩篮球"){
								    url  = "/Trade/Jclq/view.htm?id=";
								}
								tr += "<a href='"+url+$(this).attr("cid")+"'><div class='bh t_c'>"+$(this).attr("i")+"</div><div class='lottname t_c'>"+$(this).attr("lotname")+"</div><div class='money color_red t_c'>"+$(this).attr("allmoney")+"<%=setting(56)%></div><div class='zj t_c'>"
								if($(this).attr("jj")>0)
								{
