...
conn.execute(sql)
Response.Write "<script>alert('兑换密码设置成功！');location.href='Drawal.html';</script>"
end if
%>
<SCRIPT language=javascript>
function check(form){
...
<div class="head"><h2 id="navtit">设置兑换密码</h2><a href="javascript:history.go(-1);" class="btn-qgkj" id="sourceUrl"><span><em></em></span>返回</a></div>
...
<form name="form1" onSubmit="return check(this);" action="Securityinfo.asp?action=QKPassword" method="post">
	...
	<input class="login_btn" type="submit" value="设置兑换密码">
</form>
</div>
</div>
<%end Sub
...
'response.Write("userpass1:"&rs("userpass1")&"<br>")
'response.Write("truename:"&truename&"<br>")
'response.Write("identityid:"&identityid&"<br>")
...
'response.Write("bank_area:"&rs("bank_area")&"<br>")
'response.Write("bank_name:"&rs("bank_name")&"<br>")
'response.Write("bank_bank:"&rs("bank_bank")&"<br>")
'response.Write("bank_num:"&rs("bank_num")&"<br>")

if rs("userpass1")<>"" and truename<>"" and email<>"" and tel<>"" and rs("bank_area")<>"" and rs("bank_name")<>"" and rs("bank_bank")<>"" and rs("bank_num")<>"" then
islockset=",islock='1'"
else
islockset=""
end if
rs.close:set rs=nothing
sql = "update KR_User set birthday='"&birthday&"',sex='"&sex&"',tel='"&tel&"',email='"&email&"',qq='"&qq&"',address='"&address&"'"&truenameset&identityidset&islockset&" where username='"&session("un")&"'"
conn.execute(sql)
